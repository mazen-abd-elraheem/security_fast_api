...
def get_annual_exemption(formula_configs: dict, default: float = 20000.0) -> float:
    """
    Annual personal tax exemption, loaded from PayrollFormulaConfig['__tax__'].
    Falls back to `default` only if the DB has no value configured yet — this
    keeps the value editable without a code change (previously hardcoded).
    """
    tax_cfg = formula_configs.get("__tax__", {})
    return tax_cfg.get("annual_personal_exemption", default)
...
def calc_overtime_pay(overtime_hours: float, daily_rate: float, deduction_rules: dict) -> float:
    ...
    This is the ONLY place overtime enters pay. Overtime must never also be
    added as a flat day-count to gross/operational days — that double-pays it
    (a 1-hour and an 8-hour overtime day would otherwise both add a full
    day's wage). Callers must add this return value into net salary.
    """
    if overtime_hours <= 0:
        return 0.0
    multiplier = 1.5
...
def compute_row(
    ...
    t_overtime     = sum(wk(i, "overtime")         for i in range(4))  # count of days w/ OT (display only)
    t_rest_allow   = sum(wk(i, "rest_allowance")   for i in range(4))
    t_late         = sum(wk(i, "late")             for i in range(4))
    t_late_minutes = sum(wk(i, "late_minutes")     for i in range(4))  # excess minutes over grace threshold
    ...
    # ── BG: gross days ──
    # NOTE: overtime is intentionally NOT added here as a flat day-count.
    # `t_overtime` (days that had *any* overtime) used to be added as if it
    # were a full extra day of pay regardless of how many overtime hours
    # were actually worked — that silently double-paid overtime once
    # `calc_overtime_pay()` also paid it by the hour below. Overtime pay is
    # computed once, in EGP, from actual hours, and added directly to net
    # salary further down.
    gross_days = work_days - t_absent_exc + t_rest_allow
    ...
    # ── Late deduction: per-minute (EGP/min on excess minutes) takes
    # priority when the DeductionRule is configured that way; otherwise
    # fall back to the day-based half-day-per-late-occurrence rule. This
    # used to be computed but never applied — per-minute rules were a
    # silent no-op. Now whichever mode is configured actually deducts.
    late_amount_per = 0.0
    if r_late and getattr(r_late, "is_per_minute", False):
        late_amount_per = r_late.amount or 0.0
        ded_late = t_late_minutes * late_amount_per
    else:
    ...
    # ── Overtime pay (hour-based — the single source of truth for OT pay) ──
    ...
    cl_total_income = cj_actual + ck_allowances + overtime_pay
    cm_ins          = ins_share
    annual_exemption = get_annual_exemption(formula_configs)
    cn_exemption    = annual_exemption / 12.0
    ...
    # ── BQ: net salary ──
    # Overtime pay is added here — it is real money owed to the employee and
    # must land in net salary, not just sit in the `overtime_pay` display
    # column as it did before.
    total_deductions_monetary = total_attendance_penalties + adv_ded + ins_share + bp_tax
    bq_net = round(
        salary_ops + annual_inc_current + annual_inc_prev + overtime_pay - total_deductions_monetary, 0
    )
    ...
    return {
        ...
        "total_late":               t_late,
        "total_late_minutes":       t_late_minutes,
        ...
        "annual_increase_base":     round(ai_base, 2),
        "annual_increase_pct":      round(pct, 4),
